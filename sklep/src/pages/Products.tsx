...
const ProductsList = () => {
  ...
  const CustomRightArrow = ({ onClick }: any) => {
    return (
      <AiOutlineArrowRight
        onClick={() => onClick()}
        className=" w-[60px] h-[100%]  absolute right-[-60px] text-black text-4xl hover:text-[#d8ca8d] transition-all duration-200"
      />
    );
  };
  ...
  const CustomLeftArrow = ({ onClick }: any) => {
    return (
      <AiOutlineArrowLeft
        onClick={() => onClick()}
        className=" w-[60px] h-[100%] absolute left-[-60px] text-black text-4xl hover:text-[#d8ca8d] transition-all duration-200"
      />
    );
  };
            ...
            {data.map((product, i) => {
              return (
                <div
                  key={i}
                  id={product.id}
                  className="product w-[98%] relative cursor-pointer bg-[#f1f1f1] pb-2 group">
