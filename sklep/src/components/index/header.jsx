...
const Header =()=>{
    ...
    useEffect(()=>{
     ...
     gsap.fromTo('.headerContentElement', {x:'100%'}, {x:'-20%', scrollTrigger:{trigger:'headerContent', start:'top bottom', scrub: true, ease:'power3',}})  
     gsap.fromTo('.headerTitleEl', {y:"100%", opacity:0}, {y:0, opacity:1, duration:.5, stagger:.2, delay:.2})
     gsap.fromTo('.headerButton', {opacity:0}, {opacity:1, duration:.3, delay:.5})
    },[])
                ...
                <div className="relative top-[50%] translate-y-[-50%] w-full text-center space-y-[40px]">
                    <div className="headerTitle space-y-4 h-fit overflow-hidden">
                        ...
                        <img src={logo} className="headerTitleEl w-[200px] mx-auto"/>
                          ...
                          <h1 className="headerTitleEl text-[#d4d4d4] font-brunoAce text-[40px]">WNĘTRZA Z KLASĄ</h1>
                    </div>
                    <button onClick={scrollToProducts} className="headerButton border-[2px] border-[#c93] text-[#c93]   w-[250px] h-[60px] font-gruppo font-extrabold text-[25px] hover:text-[#000000] hover:bg-[#fcfcfc] hover:border-white transition-all duration-200">ZOBACZ OFETRĘ
                            </button>
                </div>
            </div>
